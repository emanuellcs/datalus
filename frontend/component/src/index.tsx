...
async function runDdim(args: ComponentArgs): Promise<Record<string, unknown>[]> {
  const precision = String(args.conditions?.precision ?? "model_int8.onnx")
  const modelBytes = await loadArrayBuffer(`${args.artifactBaseUrl}/${precision}`)
  ...
  for (let i = args.ddimSteps - 1; i >= 0; i -= 1) {
    ...
    const feeds: Record<string, ort.Tensor> = {
      ...
      [timestep]: new ort.Tensor("int64", BigInt64Array.from({ length: args.nRecords }, () => BigInt(t)), [args.nRecords]),
    }
    ...
    for (let j = 0; j < total; j += 1) {
      const predX0 = (x[j] - Math.sqrt(1 - alphaT) * noise[j]) / Math.sqrt(alphaT)
      next[j] = Math.sqrt(alphaPrev) * predX0 + Math.sqrt(Math.max(1 - alphaPrev, 0)) * noise[j]
    }
    x = next
  }
  ...
  return Array.from({ length: args.nRecords }, (_, row) => {
    ...
    columns.forEach((column, idx) => {
      record[column] = Number(x[row * latentDim + (idx % latentDim)].toFixed(6))
    })
    return record
  })
}
...
createRoot(document.getElementById("root") as HTMLElement).render(<Connected />)
